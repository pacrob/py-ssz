...
from ssz.sedes.basic import BasicSedes  # type: ignore


class UInt(BasicSedes[int, int]):  # type: ignore
    ...
    def deserialize(self, data: bytes) -> int:
        if len(data) != self.size:
            raise DeserializationError(
                f"Cannot deserialize length {len(data)} byte-string "
                f"as uint{self.size*8}"
            )
