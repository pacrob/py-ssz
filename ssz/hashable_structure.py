...
def update_element_in_chunk(
    ...
    This function replaces the element given by its index in the chunk with the given data.  # noqa: E501

    If the length of the element is zero or not a divisor of the chunk size,
    a `ValueError` is raised. If the index is out of range, an `IndexError` is raised.
...
def get_num_padding_elements(
    *, num_original_elements: int, num_original_chunks: int, element_size: int
    ...
    """Compute the number of elements that would still fit in the empty space of the last chunk."""  # noqa: E501
...
def get_updated_chunks(
    *,
    ...
    The changeset is given as a dictionary of element indices to updated elements and
    a sequence of appended elements. Note that appended elements that do not affect
    existing chunks are ignored.

    The pre-existing state is given by the sequence of original chunks and the number
    of elements represented by these chunks.
...
class BaseHashableStructure(HashableStructureAPI[TElement]):
    ...
    def from_iterable_and_sedes(
        ...
        if max_length and len(elements) > max_length:
            raise ValueError(
                f"Number of elements {len(elements)} exceeds maximum length {max_length}"  # noqa: E501
            )
    ...
    def __eq__(self, other: Any) -> bool:
        # hashable structures are equal if they use the same sedes and have the same root  # noqa: E501
...
class HashableStructureEvolver(HashableStructureEvolverAPI[TStructure, TElement]):
    ...
    def persistent(self) -> TStructure:
        ...
        elements = self._original_structure.elements.mset(
            *itertools.chain.from_iterable(self._updated_elements.items())
